...
/**
 * Tests for {@link OnnxMojoModelParser}.
 */
public class OnnxMojoParserTest {
    /** Test model resource name. */
    private static final String TEST_MODEL_RESOURCE = "mojos/gbm_prostate.zip";

    /** Parser. */
    private final OnnxMojoModelParser parser = new OnnxMojoModelParser();
    ...
    public void testParseAndPredict() {
        URL url = OnnxMojoParserTest.class.getClassLoader().getResource(TEST_MODEL_RESOURCE);
        if (url == null)
        ...
        try (OnnxMojoModel mdl = mdlBuilder.build(reader, parser)) {
